...
class EncryptedFTLGuestModel(PlainFTLGuestModel):
    ...
    def receive_components(self, components):
        ...
        self.uB_overlap=components[3]#New!!!just for testing difference with the plaintext(for loss)
...
class EncryptedFTLHostModel(PlainFTLHostModel):
    ...
    def send_components(self):
        if self.is_min_gen_enc:
            ...
            return [enc_uB_overlap, enc_uB_overlap_2, enc_mapping_comp_B,self.uB_overlap]#New!!!just for testing difference with the plaintext(for loss)
        else:
