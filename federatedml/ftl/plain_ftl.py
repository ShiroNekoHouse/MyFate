...
class PlainFTLGuestModel(PartyModelInterface):
    ...
    def __compute_phi(self, uA, y):
        ...
        # phi = sum(y*uA)/NA，y是1*NA的矩阵，uA是NA*d的矩阵，np.sum(y * uA, axis=0)得到的是1*NA的矩阵
        # np.expand_dims 、 np.sum(A,axis) 的用法
    ...
    def _compute_components(self):
        ...
        self.phi_2 = np.matmul(self.phi.transpose(), self.phi)  # phi 平方

        # y_overlap and y_overlap_2 have shape (len(overlap_indexes), 1)
        self.y_overlap = self.y[self.overlap_indexes]
        # overlap_indexes是重叠部分，y是重叠部分的标签，overlap_indexes has the shape (len(overlap_indexes), 1)
