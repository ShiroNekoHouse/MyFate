...
from federatedml.ftl.OT_encrypted_ftl import OTEncryptedFTLHostModel
...
class HeteroOTEncryptFTLhost(HeteroFTLHost):
    def __init__(self, host, model_param: FTLModelParam, transfer_variable: HeteroFTLTransferVariable):
        super(HeteroOTEncryptFTLhost, self).__init__(host, model_param, transfer_variable)
        self.host_model: OTEncryptedFTLHostModel = host
    ...
    def fit(self, host_data):
        ...
        while self.n_iter_ < self.max_iter:
            ...
            #test test 1 line
            self.host_model._update_gradients()


            #New!!!计算guest的loss_share，发送给guest
            host_loss_share=self.host_model.send_loss_shares()
            self._do_remote(host_loss_share, name=self.transfer_variable.host_loss_share.name,
                            tag=self.transfer_variable.generate_transferid(self.transfer_variable.host_loss_share, self.n_iter_),
                            role=consts.GUEST,
                            idx=-1)
            #New!!!计算guest的gradient_share，发送给guest
            host_gradient_share_to_guest=self.host_model.send_gradient_shares()
            self._do_remote(host_gradient_share_to_guest, name=self.transfer_variable.host_gradient_share_to_guest.name,
                            tag=self.transfer_variable.generate_transferid(self.transfer_variable.host_gradient_share_to_guest, self.n_iter_),
                            role=consts.GUEST,
                            idx=-1)
            #New!!!!OT计算gradient_share，收集guest的share，合并计算gradient
            self.host_model.prepare_gradient()
            guest_gradient_share_to_host = self._do_get(name=self.transfer_variable.guest_gradient_share_to_host.name,
                                      tag=self.transfer_variable.generate_transferid(self.transfer_variable.guest_gradient_share_to_host, self.n_iter_),
                                      idx=-1)[0]
            self.host_model.compute_gradients(guest_gradient_share_to_host)
    ...
    def create(cls, ftl_model_param: FTLModelParam, transfer_variable: HeteroFTLTransferVariable, ftl_local_model):
        if ftl_model_param.is_encrypt:
            ...
            #new
                ...
                host_model = OTEncryptedFTLHostModel(local_model=ftl_local_model, model_param=ftl_model_param)
                host = HeteroOTEncryptFTLhost(host_model, ftl_model_param, transfer_variable)
            else:
                # encrypted ftl host
