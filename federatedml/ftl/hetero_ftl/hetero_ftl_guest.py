...
from federatedml.ftl.OT_encrypted_ftl import OTEncryptedFTLGuestModel
...
class HeteroOTEncryptFTLGuest(HeteroFTLGuest):

    def __init__(self, guest, model_param: FTLModelParam,
                 transfer_variable: HeteroFTLTransferVariable):
        super(HeteroOTEncryptFTLGuest, self).__init__(guest, model_param, transfer_variable)
        self.guest_model: OTEncryptedFTLGuestModel = guest
    ...
    def fit(self, guest_data):
        ...
        while self.n_iter_ < self.max_iter:
            ...
            #test test 2 line
            self.guest_model._update_gradients()
            self.guest_model._update_loss()

            #New!!!!OT计算share，收集host的share，合并计算loss
            self.guest_model.prepare_loss()
            host_loss_share = self._do_get(name=self.transfer_variable.host_loss_share.name,
                                     tag=self.transfer_variable.generate_transferid(
                                         self.transfer_variable.host_loss_share, self.n_iter_),
                                     idx=-1)[0]
            self.guest_model.compute_loss(host_loss_share)
            #New!!!!OT计算share，收集host的share，合并计算gradient
            self.guest_model.prepare_gradient()
            host_gradient_share_to_guest = self._do_get(name=self.transfer_variable.host_gradient_share_to_guest.name,
                                     tag=self.transfer_variable.generate_transferid(
                                         self.transfer_variable.host_gradient_share_to_guest, self.n_iter_),
                                     idx=-1)[0]
            self.guest_model.compute_gradients(host_gradient_share_to_guest)
            #New!!!!OT计算host的share，发送给host
            guest_gradient_share_to_host=self.guest_model.send_gradient_shares()
            self._do_remote(guest_gradient_share_to_host, name=self.transfer_variable.guest_gradient_share_to_host.name,
                            tag=self.transfer_variable.generate_transferid(self.transfer_variable.guest_gradient_share_to_host,
                                                                           self.n_iter_),
                            role=consts.HOST,
                            idx=-1)
    ...
    def create(cls, ftl_model_param: FTLModelParam, transfer_variable: HeteroFTLTransferVariable, ftl_local_model):
        if ftl_model_param.is_encrypt:
            ...
            #new
                ...
                # encrypted OT guest
                LOGGER.debug("@ create encrypt OT guest")
                guest_model = OTEncryptedFTLGuestModel(local_model=ftl_local_model, model_param=ftl_model_param)
                guest = HeteroOTEncryptFTLGuest(guest_model, ftl_model_param, transfer_variable)
            else:
                # encrypted ftl guest
